// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot;

/** Add your docs here. */
public class VoltageConstants {
    // Intake
    public static final double vk_IntakeForward = 6.0;
    public static final double vk_IntakeReverse = -6.0;

    // Kicker
    public static final double vk_KickerForward = 8.0;
    public static final double vk_KickerReverse = -3.0;

    // Shooter
    public static final double vk_TopShooterForward = 8.0;
    public static final double vk_BottomShooterForward = 8.0;

    // Arm
    public static final double vk_ArmUp = -6.0;
    public static final double vk_ArmDown = 6.0;

    // Shooter From Sub
    public static final double vk_TopShooterSub = 6.0;
    public static final double vk_BottomShooterSub = 6.0;

    // Shooter From Podium
    public static final double vk_TopShooterPodium = 8.0;
    public static final double vk_BottomShooterPodium = 8.0;

    // Shooter for Amp
    public static final double vk_TopShooterAmp = .5;
    public static final double vk_BottomShooterAmp = 3.;
    // Shooter for Back Podium
    public static final double vk_TopShooterMax = 10.5;
    public static final double vk_BottomShooterMax = 10.5;

    // Climber up
    public static final double vk_RightClimberUp = 10.0;
 

    // Climber down
    public static final double vk_RightClimberDown = -10.0;
   

}
