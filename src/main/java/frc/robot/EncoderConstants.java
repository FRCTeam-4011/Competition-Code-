// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot;

/** Add your docs here. */
public class EncoderConstants {
    public static final double ek_IntakeArmPOS = 0.0;
    public static final double ek_PodiumShotArmPOS = -9.50;
    public static final double ek_BackPodiumShotArmPOS = -4.35;
    public static final double ek_PodiumShotArmPOSAuto = -9.50;
    public static final double ek_AmpShotArmPOS = -18.60;
    public static final double ek_SubShotArmPOS = -18.60;// change to -19.04
    public static final double ek_HomeArmPOS = -0.10;
    public static final double ek_CustomArmPOS = 0.0;
    public static final double ek_LeftClimberMaxUp = 0.0;
    public static final double ek_RightClimberMaxUp = 0.0;
    public static final double ek_LeftClimberMaxDown = 0.0;
    public static final double ek_RightClimberMaxDown = 0.0;

}
